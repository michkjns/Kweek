...
//#define HEADLESS // oo man, too bad, physx doens't work too wel under WINE ...
...
{
	HWND hApp = FindWindow( NULL, "Kweek" );
...
{  
	...
	//	printf( "Reading arguments\n" );
		...
		if( argc == 3 )
		{
			if( strcmp( argv[2], "headless" ) == 0 )
			{
		//		printf( "I am a headless server\n" );
				isHeadless = true;
			}
		}
	}
	...
	if( !isHeadless )
	{
		...
		if (!init()) 
		{
			...
			surface->InitCharset();
			vbo = false;
			SDL_WM_SetCaption( "Kweek - FALLBACK", NULL );
		}
		else SDL_WM_SetCaption( "Kweek", NULL );
	}
#endif
	int exitapp = 0;
	game = new Game();
	...
	while (!exitapp) 
	{
		...
		while (SDL_PollEvent( &event )) 
		{
			...
			case SDL_MOUSEMOTION:
				...
				game->MouseMoveRel( event.motion.xrel, event.motion.yrel );
				break;
